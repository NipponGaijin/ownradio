CREATE OR REPLACE FUNCTION public.attachdevicetouser(
	device_id text,
	googleemail text,
	googleidtoken text,
	OUT userid uuid,
...
-- 			Если устройство найдено в БД, проверяем найден ли пользователь с email
		IF deviceid IS NOT NULL THEN
			...
			INTO useruuid
			FROM public.users
			WHERE public.users.email = googleemail ORDER BY reccreated DESC;
...
ALTER FUNCTION public.attachdevicetouser(text, text, text)
    OWNER TO postgres;
